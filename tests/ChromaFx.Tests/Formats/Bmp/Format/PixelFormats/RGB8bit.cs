...
public class Rgb8Bit : FormatBaseFixture
{
    ...
    public override IPixelFormat Format => new IO.Formats.Bmp.Format.PixelFormats.Rgb8Bit();

    [Fact]
    public void Decode()
    {
        var paletteData = new byte[1024];
        var tempPalette = new IO.Formats.Bmp.Format.Palette(256, paletteData);
        using var tempFile = System.IO.File.Open(FileName, FileMode.Open, FileAccess.Read);
        var data = Format.Read(new IO.Formats.Bmp.Format.Header(44, 40, 0, 1760, 0, 0, 0, 0, Compression.Rgb), tempFile);
        data = Format.Decode(new IO.Formats.Bmp.Format.Header(44, 40, 0, 1760, 0, 0, 0, 0, Compression.Rgb), data, tempPalette);
    ...
    public void Encode()
    {
        ...
        var data = Format.Read(new IO.Formats.Bmp.Format.Header(44, 40, 0, 1760, 0, 0, 0, 0, Compression.Rgb), tempFile);
        data = Format.Decode(new IO.Formats.Bmp.Format.Header(44, 40, 0, 1760, 0, 0, 0, 0, Compression.Rgb), data, tempPalette);
        data = Format.Encode(new IO.Formats.Bmp.Format.Header(44, 40, 0, 1760, 0, 0, 0, 0, Compression.Rgb), data, tempPalette);
