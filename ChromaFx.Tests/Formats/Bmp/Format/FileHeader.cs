...
public class FileHeader
{
    ...
    public void CreateByteArray()
    {
        ...
        var testFileHeader = new ChromaFx.Formats.Bmp.Format.FileHeader(data);
        Assert.Equal(1000, testFileHeader.FileSize);
        Assert.Equal(54, testFileHeader.Offset);
        Assert.Equal(0, ChromaFx.Formats.Bmp.Format.FileHeader.Reserved);
        Assert.Equal(19778, ChromaFx.Formats.Bmp.Format.FileHeader.Type);
    }

    [Fact]
    public void CreateValues()
    {
        var testFileHeader = new ChromaFx.Formats.Bmp.Format.FileHeader(1000, 54);
        Assert.Equal(1000, testFileHeader.FileSize);
        Assert.Equal(54, testFileHeader.Offset);
        Assert.Equal(0, ChromaFx.Formats.Bmp.Format.FileHeader.Reserved);
        Assert.Equal(19778, ChromaFx.Formats.Bmp.Format.FileHeader.Type);
    ...
    public void Read()
    {
        ...
        var testFileHeader = ChromaFx.Formats.Bmp.Format.FileHeader.Read(stream);
        Assert.Equal(1000, testFileHeader.FileSize);
        Assert.Equal(54, testFileHeader.Offset);
        Assert.Equal(0, ChromaFx.Formats.Bmp.Format.FileHeader.Reserved);
        Assert.Equal(19778, ChromaFx.Formats.Bmp.Format.FileHeader.Type);
